...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare live ResQ generated datasets to fresh Arco data-engine output."
    )
    parser.add_argument(
        "--report-dir",
        type=Path,
...
def open_progress_window(progress_path: Path) -> bool:
    ...
    title = f"Arco Validation Progress - {TARGET_PROJECT_NAME}".replace("'", "''")
    command = (
