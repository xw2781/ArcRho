...
# Description: List every reserving class in the active project, let the user pick any set in a review table, choose whether to merge with or overwrite the existing ArcRho copies, then import each selected class from ResQ through the ArcRho Bridge one at a time with batch progress and a final summary.
# Scope: Project
# </arcrho-macro>
...
def run_macro(active_dfm=None, active_context=None):
    ...
    # One merge-or-overwrite decision covers the whole batch; the sibling
    # macro owns the prompt so both imports word and confirm it identically.
    try:
        import_policy = single.choose_import_policy(
            ui,
            title=TITLE,
            scope_note=(
                f"Project: {project_name}\n"
