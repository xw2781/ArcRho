...
# Version: 1.8.0
# Release Note: A DFM now imports its tail: each average row's "- Ult" value comes from that row's ResQ tail factor instead of being reset to 1, and the ResQ Curves tab (Future Dev. Periods, Free Fit C, Include flags, user value columns, and the Selected Estimate Number per period and for the tail) lands on the new ArcRho Curves tab.
