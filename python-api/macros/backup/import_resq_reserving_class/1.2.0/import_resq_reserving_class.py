...
# Version: 1.2.0
# Release Note: Send reserving-class imports through the shared ArcRho Bridge.
...
def _logical_rc_path(value: object) -> str:
    ...
    if not raw or path.is_absolute() or path.drive:
        raise ValueError("Reserving-class path must be a relative ArcRho path.")
    parts = tuple(part for part in path.parts if part not in {"", "."})
    if not parts or any(part == ".." for part in parts):
        raise ValueError("Reserving-class path must not contain parent-path segments.")
    return "\\".join(parts)
...
def discover_live_bridge_workers(
    ...
    *,
    ...
    for path in candidates:
        ...
        if role != "bridge_worker" or not _is_true(payload.get("ResQGuiRunning")):
            continue
        live.append(path)
...
def _request_paths(server_root: object, request_id: str) -> tuple[Path, Path]:
    root = Path(server_root)
    request_dir = root / REQUEST_ROOT / "requests"
    status_dir = root / REQUEST_ROOT / "statuses"
