...
# Version: 1.0.0
# Release Note: Initial release: preview and selectively apply timestamp-based ArcRho/ResQ synchronization.
...
def _load_runtime_modules():
    ...
    try:
        ...
        shared_root = Path(get_server_root(required=True)) / "shared"
        roots.extend((shared_root / "macros", shared_root / "python-api"))
    except Exception:
        pass

    def find(relative_candidates: tuple[Path, ...], label: str) -> Path:
        for root in roots:
        ...
        raise FileNotFoundError(f"Could not locate {label}. Searched:\n{searched}")

    migration_script = find(
        (
            Path("python-api/migration/resq_data_migration.py"),
            Path("migration/resq_data_migration.py"),
            Path("resq_data_migration.py"),
        ),
        "the ResQ migration runtime",
    )
    export_macro = find(
        (
            Path("python-api/macros/export_reserving_class_to_resq.py"),
            Path("macros/export_reserving_class_to_resq.py"),
            Path("export_reserving_class_to_resq.py"),
        ),
        "the ArcRho-to-ResQ exporter",
    )
...
def collect_resq_inventory(runtime: Mapping[str, Any], exporter) -> list[dict[str, Any]]:
    ...
    for collection_kind, factory in collections:
        ...
        for obj in _iter_resq_collection(collection, collection_kind):
            ...
            import_supported = kind in {KIND_DATASET, KIND_BS_SR, KIND_BS_CRA} and known_type
...
def _row_tone(row: Mapping[str, Any]) -> str:
    ...
    if "unsupported" in status or "unknown" in status:
        return "muted"
    if row.get("action") == "arcrho_to_resq":
        return "arcrho"
    if row.get("action") == "resq_to_arcrho":
        return "resq"
...
def _verify_dataset_export(
    ...
    if is_triangle:
        ...
        if int(getattr(target, "OriginCount")) < len(values):
            raise RuntimeError("ResQ triangle has fewer origin rows than the ArcRho cache.")
        for origin_index, source_row in enumerate(values, start=1):
            ...
            for development_index, expected in enumerate(source_row[:width], start=1):
                if expected is None:
                    continue
...
def _preflight_method_export(exporter, row: Mapping[str, Any]) -> None:
    ...
    if kind == KIND_DFM:
        ...
        require(details.get("input triangle"), exporter._find_triangle, "DFM input triangle")
        if target is None:
            return
        ...
        require(method_tab.get("latest_dataset"), find_triangle_or_vector, "latest")
        require(method_tab.get("dfm_dataset"), exporter._find_vector, "percentage-developed")
        ...
        if priors:
            prior = priors[0] if isinstance(priors[0], Mapping) else {}
            require(prior.get("name"), exporter._find_vector, "prior")
    elif kind == KIND_CC:
        require(method_tab.get("exposure_dataset"), exporter._find_vector, "exposure")
        require(method_tab.get("latest_dataset"), exporter._find_triangle, "latest")
        require(method_tab.get("prior_ultimate_dataset"), exporter._find_vector, "prior-ultimate")
    elif kind == KIND_RS:
...
def _resq_import_target(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "method_names": [
            str(item.get("resq_method_name") or ""),
            str(row.get("name") or ""),
        ] if kind != KIND_DATASET else [],
        "display_kind": kind,
    }
    ...
    *,
    ...
    def record(row: Mapping[str, Any], success: bool, message: str) -> None:
        ...
        if success:
            ...
            if row.get("action") == sync_contract.ACTION_RESQ_TO_ARCRHO:
                imported_names.append(str(row.get("name") or ""))
        results.append({
            "id": row.get("id"),
            "name": row.get("name"),
            "kind": row.get("kind"),
    ...
    with io_lock, lease_context, tempfile.TemporaryDirectory(
        ...
        with nullcontext():
            ...
            if data_local:
                try:
                    exporter.project.BeginDelayedUpdate()
                    delayed = True
                except Exception:
                    delayed = False
            try:
                ...
                if delayed:
                    exporter.project.EndDelayedUpdate()
                    delayed = False
            except Exception:
                if delayed:
                    try:
                        exporter.project.CancelDelayedUpdate()
                    except Exception:
                        pass
                    delayed = False
                raise
            ...
            data_imported_count = len(imported_names)
            if data_imported_count:
                ...
                propagation = _refresh_dfm_dependents_for_sources_locked(
                    reserving_class, imported_names[:data_imported_count]
                )
            ...
            # Dataset imports can recalculate method outputs. Re-inventory after
            # that canonical walk and skip any reviewed method that moved.
            method_rows = method_local + method_remote
            if method_rows:
                current = _plan_context(runtime, project_name, rc_path, server_root, exporter=exporter)
                current["plan"] = sync_contract.build_sync_plan(
                    current["arcrho"], current["resq"], locked_observation["state"]
                )
                stale_methods = set(_stale_selected_rows(runtime, method_rows, current["plan"]))
            else:
                stale_methods = set()

            for row in method_local:
                if str(row.get("name") or "") in stale_methods:
                    clear_pending_keys.append(str(row.get("key") or ""))
                    record(row, False, "Timestamp changed during dataset synchronization; rerun the review.")
                    continue
                try:
                    ok, message = _export_one_to_resq(exporter, row)
                except Exception as exc:
            ...
            method_imported_names = imported_names[data_imported_count:]
            if method_imported_names:
                migration.refresh_sidecar_graphs_for_rc(rc_dir)
                reserving_class = ArcRhoClient(server_root).project(project_name).reserving_class(rc_path)
                propagation = _refresh_dfm_dependents_for_sources_locked(
                    reserving_class, method_imported_names
                )
                for warning in propagation.warnings:
                    results.append({
                        "id": "",
