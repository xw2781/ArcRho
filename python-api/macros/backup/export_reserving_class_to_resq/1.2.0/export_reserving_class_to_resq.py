...
# Version: 1.2.0
# Release Note: Push a DFM's Method Notes from its ArcRho output sidecar into the ResQ method Notes, with line breaks ResQ renders.
...
class ResQReservingClassExporter:
    ...
    def _export_dfm(self, name, details, payload, entry):
        ...
        notes = self._sync_dfm_method_notes(dfm, entry)
        dfm.Save()
    ...
    def _sync_dfm_method_notes(self, dfm, entry):
        # An entry carries ``notes`` only when its output sidecar, the ArcRho
        # owner of Method Notes, was readable: an absent field leaves the ResQ
        # Notes unchanged and an empty value clears them.
        if "notes" not in entry:
            return 0
        notes = self._resq_notes_text(entry["notes"])
        if str(getattr(dfm, "Notes", "") or "") == notes:
            return 0
        dfm.Notes = notes
        return 1
