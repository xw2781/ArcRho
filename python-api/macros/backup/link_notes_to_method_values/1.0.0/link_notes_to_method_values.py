...
# Version: 1.0.0
# Release Note: Initial release: rewrite the labels typed by hand in the DFM Notes as {...} placeholders that read the same value back from the method.
...
def link_notes_to_method_values(notes: str, payload: Any) -> tuple[str, list[tuple[str, str]]]:
...
def _summary(replacements: list[tuple[str, str]]) -> str:
    if not replacements:
        return "No text in these notes matches a value the method can supply."
    ...
    return f"Linked {len(replacements)} phrase(s) to method values: {shown}{more}."
...
def run_macro(active_dfm=None, active_context=None):
    ...
    linked_notes, replacements = link_notes_to_method_values(original_notes, payload)
    summary = _summary(replacements)
    if replacements:
