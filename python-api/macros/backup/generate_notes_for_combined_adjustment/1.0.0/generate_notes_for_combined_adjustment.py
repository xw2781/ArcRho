...
def _factor_lines(factors: list[dict[str, Any]]) -> tuple[list[str], list[str], bool]:
    ...
    for factor in factors:
        ...
        if factor["kind"] == "reference":
            ...
            location = f" ({factor['location']})" if factor.get("location") else ""
        else:
        ...
        lines.append(f"  - Apply {description} of {percent}{location} = {multiplier};")
...
def _clear_generated_notes(dfm: Any) -> None:
    lines = [
        line
        for line in dfm.notes.splitlines()
        if not any(line.strip().startswith(prefix) for prefix in GENERATED_LINE_PREFIXES)
    ]
    text = "\n".join(lines)
    text = re.sub(r"\n{3,}", "\n\n", text).strip()
        ...
        if parsed["ok"]:
            for factor in parsed["factors"]:
                if factor["kind"] == "reference":
                    ...
                    factor["location"] = (
                        _reference_location(resolved, factor["reference"]) if resolved else ""
                    )
                else:
