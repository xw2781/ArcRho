...
- **Step 22 (DFM tails):** the calculation change ships with the server redeploy (step 21) and the DFM page change with the next app release. Until that release, saving a DFM from the released app still resets a typed tail on a computed row to 1.0, as it does today; nothing it could open breaks.
...
| 22 | A tail typed on any average row in a DFM is kept, as in ResQ (runs before step 20) | [x] | 2026-09-23 | 70 min | 16 min | A tail factor typed on any average row of a DFM, Volume - all included, is now kept and used, and the tail cell of every row takes a typed value, as in ResQ; no saved result changed. |
| 20 | The MP+PIP bootstrap carries ResQ's tail factor, so its oldest years get their range | [x] | 2026-09-23 | 55 min | 17 min | MP+PIP now carries ResQ's tail factor, so its oldest years have a range again and the Total's oldest years match ResQ's; the parity check now compares every year, not just the totals. |
| 21 | The server components carry the tail-factor fix | [x] | 2026-09-23 | 25 min | 6 min | The shared server now keeps a tail typed on any average row when it imports from ResQ or saves a DFM, so MP+PIP's oldest years keep their range after the next import. |

Overall: 21 of 21 steps done (step 3 dropped). Estimated 1,320 min, actual 330 min.
...
Decisions 1, 2, 3 and 4 were settled on 2026-09-23; none is open.

**Decision 4 (settled 2026-09-23 by the user: option A, "the tail entry should follow ResQ's behavior"): may a computed average row carry its own tail factor?** Yes: every average row's tail cell is an input, as in ResQ. Step 22 makes the change; step 20 then re-imports `MP+PIP`, and step 21 deploys. Matching ResQ's `MP+PIP` `F 25` needs a change to how Arco calculates a DFM, which step 20's guard forbids it to make alone.
...
- **1.0018 is typed in, not calculated** (checked over COM 2026-09-23, read only). It sits in the tail cell of the `Volume - all` row on the Ratios tab. No fitted curve produces it: the Curves tab's fitted tails are 1.000095 (exponential decay), 1.000209 (inverse power), 1.000109 (power) and 1.0000006 (Weibull), with one future period. The Curves tab's tail selection is Initial Selection, which only relays the Ratios tab's selected row and its tail. In ResQ the tail cell of every average row is an input, which is what option A gives Arco. COL's and BI Total's `F 25` carry 1.0 on every computed row.
...
- **What a change would move.** A read-only scan of every stored DFM under `E:\ArcRho Server\projects` found 4,333 method files. None of the 3,877 in the current shape holds a computed row with a tail other than 1.0, because the recalculation has always forced it. The other 456 are in an older shape the scan did not parse. So keeping a stored tail on computed rows changes no stored result until a class is re-imported from ResQ or someone enters a tail by hand. Step 22 re-ran the scan with the older shapes parsed (4,377 files that day): the 3,877 current-shape files hold 31,323 computed rows, none with a tail other than 1.0. The 500 older-shape files (460 with spaced tab names, 40 flat) hold 88 computed rows with another tail, all ResQ prior-analysis rows (`Aug 2024`, or a prior method's name) that the older import filed as simple averages, in `NJ_Annual_Prod_2026 Jul` (83), `Jun` (4) and `Q1-Feb` (1). None of them is selected for the tail column, and the app no longer reads that shape, so no stored result moves and no republish is needed.
...
### Step 22 — A tail typed on any average row is kept, as in ResQ (runs before step 20)

**Goal.** Per decision 4, a DFM keeps the tail factor stored on any average row, computed rows included (1.0 when none is stored), in the calculation and on the DFM page, and the tail cell of a computed row is editable, as in ResQ.

**Read first.** Decision 4 under [Open decisions](#open-decisions), including the finding that ResQ's 1.0018 is a typed value; [dfm_contract.py](../../../python-api/src/arcrho_api/dfm_contract.py) around `_calculate_formula_values` and the stored-tail handling (about lines 1500-1560); [dfm_state.js](../../../frontend/ui/method_pages/dfm/dfm_state.js) `summaryRowOwnsTail` (about lines 679-694) and whatever renders the Ratios tab's tail column; the export macro's `_sync_dfm_tail_factors` in `python-api/macros/export_reserving_class_to_resq.py`; memories `resq-dfm-curves-com-api`, `bf-cc-formula-change-needs-republish`, `method-precision-observed-not-projected`, `frontend-node-test-suite`, `arcrho-dev-ui-cache-restart`; `$arcrho-json-contract` (no field changes shape, but confirm it). Also needed (added by step 22): `hydrateUserEntryValuesFromAverageFormulaValues` and `buildAverageFormulaValues` in [dfm_persistence.js](../../../frontend/ui/method_pages/dfm/dfm_persistence.js) (only User Entry and benchmark rows were given their stored values on open); the tail branch of `updateRatioSummary` in [dfm_ratios_summary_table.js](../../../frontend/ui/method_pages/dfm/dfm_ratios_summary_table.js); `updateSummaryFormulaBarForCell` and `commitSummaryFormulaInput` in [summary_entries.js](../../../frontend/ui/method_pages/dfm/ratios_summary/summary_entries.js) and the `=` key handler in [summary_interactions.js](../../../frontend/ui/method_pages/dfm/ratios_summary/summary_interactions.js), which are where a summary cell becomes editable.

**Do.**
- [x] Contract: a computed average row keeps its stored tail instead of being reset to 1.0; nothing else about averages changes. The selected tail follows as today from the selected row.
- [x] DFM page: the same rule in its state logic, and the Ratios tab's tail cell on a computed row accepts a typed value the way a User Entry row's does. Bump the `?v=` stamps of the changed modules. A computed or benchmark row's tail takes a number (or plain arithmetic) through the formula bar; a row, dataset or workbook reference is refused, because nothing would recalculate it. `dfm_state.js` is imported without a stamp everywhere and stays so.
- [x] Confirm the export writes a computed row's tail to ResQ's `TailFactor` unchanged (no macro change expected; if one is needed, follow the macro rules in `python-api/macros/README.md`). Confirmed: `_sync_dfm_tail_factors` and the sync session's verification write and check every row's last value whatever its type.
- [x] Re-run the read-only scan of every stored DFM under `E:\ArcRho Server\projects`, parsing the older-shape files too, and record how many stored results the change would move (expected none). If any would move, stop and record them under Open decisions rather than shipping. None moves; see decision 4.
- [x] Update the DFM UI doc and app-server domain doc, and add a user-facing release fragment. The DFM JSON format doc's tail sentence was updated too; no field changes shape.

**Tests.** Contract test: a computed row with a stored tail keeps it and the ultimate reflects it; with none stored it is 1.0; revisions of an existing method with all-1.0 computed tails are unchanged. Node test for the page rule and the editable cell. Existing DFM contract, DFM curves parity (`dfm_curves_resq_c12.json`) and DFM frontend tests pass; the Node suite shows no new failures.

**Done when.** Tests pass, the scan result is recorded, and recalculating Arco's `MP+PIP` `F 25` with 1.0018 on `Volume - all` gives ResQ's 2017 ultimate (119,992.37) to 1e-9.

**Result (2026-09-23).** Recalculating the stored `MP+PIP` `F 25` unchanged reproduces its stored ultimates exactly; with 1.0018 typed on `Volume - all`, the row keeps it and every origin's ultimate equals ResQ's `F 92` in the step 1 fixture (the `F 25` ultimate, full precision) to a worst relative 7.7e-16, 2017 at 119,992.370285 against 119,776.772 before. Contract tests (3 new), DFM curves, bootstrap and consolidation contract and import tests (115), the export macro and ResQ DFM tests, and the app server's DFM service tests (57) pass. The Node suite has 24 failures before and after, the same set apart from the new tail-entry file (6 tests, failing only before its code) and one existing assertion on the old hydration rule, updated.

**Estimate.** Estimate: code edit 40 min, test/validation 30 min, total 70 min. Actual: code edit 9 min, test/validation 7 min, total 16 min; under a quarter of the estimate because the page's existing formula bar already carried the edit session, so the tail entry needed only a gate and a small commit path.
...
**Read first.** Step 15's result; the `resq-dfm-curves-com-api` and `resq-com-probe` memories (the Ratios-tab tail is the selected average row's `TailFactor`, the Curves-tab tail is `SelectedTailFactor`); the DFM part of the ResQ import under `python-api/migration/resq_migration/` (find where the selected ratios and the tail are written); `python-api/tests/fixtures/dfm_curves_resq_c12.json` and its test as the fixture pattern; the step 8 bootstrap import, which snapshots the DFM's selected ratios. Also needed (added by step 20): `export_dfm_output_dataset` and `_preserve_local_dfm_data` in [dfm.py](../../../python-api/migration/resq_migration/dfm.py) (an import that does not clean the class first rebases the existing Arco DFM's average rows, tails included, onto ResQ's), and the Bridge's call in [resq_import_runner.py](../../../server-components/src/arcrho_bridge/resq_import_runner.py), which always cleans.
...
- [x] Fix the import so Arco's DFM selects the same tail; a hand edit of the method file is not the fix. After step 22 the import already carries the row's tail, so expect no import change unless a gap remains. No change was needed; see the result.
- [x] Re-import `MP+PIP` from the working tree (outside the sandbox), then check that the DFM's tail and 2017 reserve (about 215) match ResQ, and that the bootstrap's 2017 and 2018 standard deviations are within the bar of ResQ's 701 and 911. Then check the Total `F 72 A` loads with `MP+PIP` changed and matches ResQ by origin after Consolidate and Save.
- [x] Extend `tools/bootstrap_parity_report.py`, or the Parity results, to compare by origin as well as in total, so a gap like this one shows up.
...
**Result (2026-09-23).** The import needed no change. `tools/resq_dfm_recording.py` runs the import's own DFM read against ResQ's live `F 25` through a proxy that records all 541 COM answers (read only; it refuses any write), and [test_resq_dfm_tail_import.py](../../../python-api/tests/test_resq_dfm_tail_import.py) replays that 5 KB recording through the real import: `Volume - all` keeps 1.0018 and is selected for the tail, and every origin's ultimate equals ResQ's own output vector to 1e-9 (2017 reserve 215.60). A copy of the recording with the tail set to 1.0 gives 2017 no reserve, so the test can catch a lost tail.

- **A re-import that does not clean the class keeps Arco's old tail.** The first re-import used step 10's partial form (`selected_names`, `cleanup_target=False`), and `F 25` stayed at 1.0. That path rebases the existing Arco DFM's average rows (definitions, selections and the whole values matrix) onto ResQ's, and since step 22 a computed row's stored tail survives the recalculation, so the old forced 1.0 wins. The app never takes that path: the Bridge always imports with `cleanup_target=True` into a staging folder, so no local DFM is rebased and ResQ's tail arrives. Re-importing `MP+PIP` whole with cleanup, as the app does, gave `F 25` 1.0018 on `Volume - all` and every ultimate equal to ResQ's to 1e-15; the `F 72 A` snapshot now carries 1.0018 as its tail. Other Arco DFMs whose computed rows still hold the old 1.0 pick up ResQ's tail on their next app import in the same way.
- **Bootstrap and consolidation.** Re-importing `Total`'s `F 72 A` consolidated it on the new `MP+PIP` bootstrap: no segment is stale and the run inputs are current. `MP+PIP`'s 2017 and 2018 standard deviations went from 0 and 618 to 771 and 966 (ResQ 701 and 911), and the Total's from 0 and 805 to 771 and 1,095 (ResQ 701 and 1,037). Only `MP+PIP` and `Total` files changed under the project.
- **By-origin parity.** The report now compares each origin's mean and standard deviation too. Every figure of the Total consolidation is within the bar; `MP+PIP` 2025 sits just outside (z −4.40 against 4.24). Eight other seeds give Arco a 2025 standard deviation of 6,131 ± 44 against ResQ's 6,279, 2.6 standard errors, so the stored run is a low draw. The by-origin view also shows gaps this step did not cause; see [Parity results](#parity-results).

Tests: the new replay test (6), and the DFM, ResQ DFM, bootstrap, consolidation and their import suites (272) pass.

**Estimate.** Estimate: code edit 30 min, test/validation 25 min, total 55 min. Actual: code edit 12 min, test/validation 5 min, total 17 min; under a third because the import needed no change and every re-import ran in under a minute.
...
**Do.**
- [x] Same procedure as step 19: listener heartbeat, `--stale`, `deploy.py`, and check the payload is only this plan's work.
- [x] Verify each component's deployed copy of the changed import modules matches the tree.

**Tests.** The deploy's own checks.

**Done when.** The components report fresh, with the fix in their deployed copies.

**Result (2026-09-23).** Step 20 changed no bundled module, so the fix this deploy carries is step 22's: the DFM contract keeping a tail typed on a computed average row, which the ResQ import and every hosted DFM save run. The listener heartbeat named `E:\XWSpace\Repos\ArcRho-buildbot` and was fresh. `--stale` listed bridge, engine, gateway and credential; as in steps 14 and 19 the deploy named the first three (`deploy.py bridge engine gateway`, request `build-260923-190401-558-xwei`, status `success`, about 3 minutes). The tree was clean, so the payload was the commits since base `13c1092e` under the component roots: ten commits, all this plan's, of which only `f9163d4e` (step 22) was new since step 19. Afterwards `--stale` reports bridge, engine and gateway `Updated` (credential still stale, as before); `dfm_contract.py` in the Bridge's importer copy and all 21 changed `frontend/app_server` and `python-api/src` files in the Engine's and Gateway's canonical copies match the tree apart from line endings; the Bridge and its worker, five Engines and the Gateway came back with fresh heartbeats, and the Gateway's health check answers `ok`.

**Estimate.** Estimate: code edit 5 min, test/validation 20 min, total 25 min. Actual: code edit 3 min, test/validation 3 min, total 6 min; about a quarter of the estimate because the warm build slots made the deploy take about 3 minutes.

## Rough size

15 steps, estimated at 1,095 minutes of agent time: 640 minutes of code editing and 455 minutes of test and validation runs. Step 3 (90 minutes: 55 editing, 35 validation) was dropped when step 2 could not identify ResQ's random stream. That leaves 14 steps and 1,005 minutes: 585 minutes of editing and 420 of validation. Step 16 (50 minutes: 30 editing, 20 validation) was added when step 10 found the import's calculated-class scope too wide, bringing the live total to 15 steps and 1,055 minutes: 615 of editing and 440 of validation. Steps 17-19 (115 minutes: 50 editing, 65 validation) were added when step 15 found the Total class missing from the project list and Simulate saving, bringing it to 18 steps and 1,170 minutes: 665 of editing and 505 of validation. Steps 20-21 (80 minutes: 35 editing, 45 validation) were added when step 15 found `MP+PIP`'s DFM missing ResQ's tail factor, bringing it to 20 steps and 1,250 minutes: 700 of editing and 550 of validation. Step 22 (70 minutes: 40 editing, 30 validation) was added when decision 4 was settled, bringing the live total to 21 steps and 1,320 minutes. All 21 were done on 2026-09-23 in 330 minutes, about a quarter of the estimate.
...
Every total statistic of every segment and of the consolidation is within the bar. **Still outside, one origin each:** 2019 in BI Total (852 against 810, +4.31), COL (156 against 169, −5.42) and PD+UMPD (177 against 191, −4.54). They were there before the fix (BI Total 2019 read +4.46), their signs disagree, and they sit in the oldest origins with only a few future cells, like the open second-oldest-origin gap above.

**Checked the same day: the 2019 flags are sampling noise.** ResQ's own 2019 standard deviation moves a lot between seeds (COL 169.4, 160.4, 160.2, 164.0; PD+UMPD 191.3, 177.1, 192.8, 179.8; BI Total 809.6, 823.4, 821.8, 829.7), and its saved run was the high end for COL and the low end for BI Total. Over four ResQ seeds (in memory, never saved) against eight Arco seeds, with error bars from the seeds themselves, 2019 is COL 163.5 ± 2.2 against 162.0 ± 0.6 (−0.9%), PD+UMPD 185.2 ± 4.0 against 182.3 ± 1.6 (−1.6%) and BI Total 821.1 ± 4.2 against 835.4 ± 5.6 (+1.7%), and every other origin of all three segments is within |z| < 2.7. The parity report's single-run bar understates the noise of these small, heavy-tailed cells (its implied standard error for COL 2019 is about half the seed-to-seed one), so a by-origin flag there needs a multi-seed check before it counts. The estimation-only second-oldest-origin difference noted above (MP+PIP 2018) was not re-checked this way; it does not show in the full model, where MP+PIP 2018 is 896 against 911.
...
| Origin | `MP+PIP` before | `MP+PIP` after | Total before | Total after |
| :--- | :--- | :--- | :--- | :--- |
| 2017 | 0 / 701 (−33.25) | 771 / 701 (+3.33) | 0 / 701 (−33.01) | 771 / 701 (+3.31) |
| 2018 | 618 / 911 (−8.00) | 966 / 911 (+1.50) | 805 / 1,037 (−9.56) | 1,095 / 1,037 (+2.40) |
| 2019 | 1,563 / 1,728 (−6.64) | 1,731 / 1,728 (+0.12) | 1,822 / 1,959 (−3.11) | 1,961 / 1,959 (+0.03) |
| 2020 | 2,801 / 2,902 (−3.53) | 2,921 / 2,902 (+0.65) | 3,080 / 3,148 (−2.43) | 3,200 / 3,148 (+1.86) |
| 2021 | 3,633 / 3,680 (−1.57) | 3,651 / 3,680 (−0.96) | 4,022 / 4,039 (−0.57) | 4,038 / 4,039 (−0.05) |
| 2022 | 4,009 / 4,175 (−4.93) | 4,131 / 4,175 (−1.30) | 4,627 / 4,741 (−2.84) | 4,729 / 4,741 (−0.29) |
| 2023 | 5,023 / 5,232 (−5.81) | 5,169 / 5,232 (−1.74) | 5,900 / 6,093 (−4.00) | 6,043 / 6,093 (−1.03) |
| 2024 | 5,151 / 5,356 (−4.90) | 5,237 / 5,356 (−2.83) | 8,701 / 8,786 (−1.39) | 8,695 / 8,786 (−1.49) |
| 2025 | 6,026 / 6,279 (−4.65) | 6,039 / 6,279 (−4.40) | 15,010 / 14,447 (+5.20) | 14,871 / 14,447 (+3.91) |
| 2026 | 28,861 / 28,561 (+1.27) | 28,891 / 28,561 (+1.39) | 70,093 / 69,543 (+1.10) | 70,285 / 69,543 (+1.48) |

In total, `MP+PIP`'s standard deviation is now 33,437 against 33,140 (+1.14) and the consolidation's 75,309 against 74,529 (+1.42); every total statistic stays within the bar.

**Verdict: the tail gap is closed.** Every by-origin figure of the Total consolidation is within the bar. `MP+PIP` 2025 is just outside on the stored seed; across eight other seeds Arco's 2025 standard deviation is 6,131 ± 44, 2.6 standard errors from ResQ's, so this is sampling.

**Gaps the by-origin view found that the tail does not explain.** They are left for a later step:

- **BI Total 2025:** 13,341 against ResQ's 12,786 (+5.75). Eight other seeds give 13,394 ± 132, about 5.6 combined standard errors above ResQ, so Arco's 2025 range for BI Total is about 4.7% wider than ResQ's, systematically. BI Total 2019 (+4.46) and 2021 (+4.27) are within sampling error across seeds (825 and 1,509 against 810 and 1,506).
- **MP+PIP 2022-2025:** across the eight seeds Arco sits 1.6-2.4% below ResQ in each of these origins (about −1.4 to −2.6 standard errors each), consistently on the low side.
- **COL 2019:** 157 against 169 (−5.01).
