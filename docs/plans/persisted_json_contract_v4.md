...
- data-engine (`py -3.10 -m pytest data-engine/tests`, `PYTHONPATH=.pytest-tools;python-api/src;data-engine/src;frontend`): 434 passed, clean; the gateway and project-duplication tests are timing flakes when suites overlap.
...
- data-engine: 433 passed; the one failure (`test_deploy_rollback` rotation) writes to the real `E:\ArcRho Server\config` and fails only while the share is down.
