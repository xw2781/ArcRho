...
| 22 | A tail typed on any average row in a DFM is kept, as in ResQ (runs before step 20) | [ ] | | 70 min | | |
| 20 | The MP+PIP bootstrap carries ResQ's tail factor, so its oldest years get their range | [ ] | | 55 min | | Waits for step 22 (decision 4 settled 2026-09-23). |
| 21 | The server components carry the tail-factor fix | [ ] | | 25 min | | |

Overall: 18 of 21 steps done (step 3 dropped). Estimated 1,320 min, actual so far 291 min.
...
**Do.**
- [ ] Contract: a computed average row keeps its stored tail instead of being reset to 1.0; nothing else about averages changes. The selected tail follows as today from the selected row.
- [ ] DFM page: the same rule in its state logic, and the Ratios tab's tail cell on a computed row accepts a typed value the way a User Entry row's does. Bump the `?v=` stamps of the changed modules.
- [ ] Confirm the export writes a computed row's tail to ResQ's `TailFactor` unchanged (no macro change expected; if one is needed, follow the macro rules in `python-api/macros/README.md`).
- [ ] Re-run the read-only scan of every stored DFM under `E:\ArcRho Server\projects`, parsing the older-shape files too, and record how many stored results the change would move (expected none). If any would move, stop and record them under Open decisions rather than shipping.
- [ ] Update the DFM UI doc and app-server domain doc, and add a user-facing release fragment.
