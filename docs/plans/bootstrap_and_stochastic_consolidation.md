...
| 20 | The MP+PIP bootstrap carries ResQ's tail factor, so its oldest years get their range | [ ] | | 55 min | | |
| 21 | The server components carry the tail-factor fix | [ ] | | 25 min | | |

Overall: 19 of 21 steps done (step 3 dropped). Estimated 1,320 min, actual so far 307 min.
...
- [ ] Fix the import so Arco's DFM selects the same tail; a hand edit of the method file is not the fix. After step 22 the import already carries the row's tail, so expect no import change unless a gap remains.
