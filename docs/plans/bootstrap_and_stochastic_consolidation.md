...
| 16 | Re-importing a segment class keeps Arco calculating its own figures; only the total class keeps ResQ's (runs before step 10) | [ ] | | 50 min | | |
...
Overall: 8 of 15 steps done (step 3 dropped). Estimated 1,055 min, actual so far 152 min.
...
Decisions 1 and 2 were settled on 2026-09-23; none is open.
...
- [ ] Replace the `Calculated`-flag decision in the import scope with that rule; delete `_resq_class_is_calculated` and anything else that only served it. The class is resolved once per class import, before any dataset is exported.
- [ ] Update the domain and migration docs that described the calculated-class behaviour.
