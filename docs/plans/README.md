...
| [excel_addin_gateway_transport.md](excel_addin_gateway_transport.md) | Investigated and decided 2026-09-12 after proving VBA can sign a Gateway request byte-exactly; broken into 9 session-sized steps covering a per-recalculation cache, the add-in's signing module, a values-returning hosted operation, the switch off the share, and the release; two decisions open, on how every Excel user gets a credential and on whether to go on now that the measured speed gain did not reproduce; 4 of 9 done, the per-recalculation cache, the add-in's signed Gateway client, the hosted operation that answers with a dataset's figures, and the same answer for a project's period headings and settings landed 2026-09-12; step 5 is written and its figures check out on both paths, but it is paused on that second decision. |
