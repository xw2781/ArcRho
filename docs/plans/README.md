...
| [excel_addin_gateway_transport.md](excel_addin_gateway_transport.md) | Investigated and decided 2026-09-12 after proving VBA can sign a Gateway request byte-exactly; broken into 9 session-sized steps covering a per-recalculation cache, the add-in's signing module, a values-returning hosted operation, the switch off the share, and the release; one decision open, on how every Excel user gets a credential; the speed question was answered on 2026-09-12 — the plan goes on because the values match and the reasons for the move do not depend on the timing; 7 of 9 done, the per-recalculation cache, the add-in's signed Gateway client, the hosted operation that answers with a dataset's figures, the same answer for a project's period headings and settings, worksheet formulas reading through the Gateway, the deletion of the share path itself, and the removal of the one producer that wrote a coarser view to disk all landed 2026-09-12, with every figure identical on both paths, the server about twice as fast on a dataset the Engine has to calculate, and the add-in now opening nothing under a project folder. |
