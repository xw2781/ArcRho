...
| [manual_input_period_rollup.md](manual_input_period_rollup.md) | Design settled 2026-09-05 and broken into ten session-sized steps with a progress table; 7 of 10 done — the shared roll-up helper is in, every dataset record carries its stored shape, every reader takes the shape its job needs, the 10,997 records already on the server have been backfilled ([run report](manual_input_period_rollup_backfill_report.md)), a project now records how fine its source data is so a generated dataset stores the shape it can be rebuilt at, a save can no longer move a hand-entered dataset's stored shape or orphan its file, and the Dataset Viewer shows that shape, offers only the views that are whole multiples of it, and is read-only at a coarser one. |
