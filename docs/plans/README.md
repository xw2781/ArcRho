...
| Plan | Status |
| :--- | :--- |
| [dataset_link_reader_period.md](dataset_link_reader_period.md) | Investigated 2026-09-17 and broken into 5 session-sized steps the same day: a dataset cell reference such as `=[A][1]` will pull its source at the period the referencing grid is shown at (a monthly source read from a yearly grid gives years), the way a method already reads its precedents, on commit and in the automatic refresh alike, then a deploy; estimated at 170 minutes of agent time; no decisions open; 3 of 5 done, the one dataset reader serving any coarser period a caller names, the resolve route reading every source at the lengths a request names, and the unattended refresh reading each source at the shape the target's links were written on, all landed 2026-09-17. |
