...
| [manual_input_period_rollup.md](manual_input_period_rollup.md) | Design settled 2026-09-05 and broken into ten session-sized steps with a progress table; 6 of 10 done — the shared roll-up helper is in, every dataset record carries its stored shape, every reader takes the shape its job needs, the 10,997 records already on the server have been backfilled ([run report](manual_input_period_rollup_backfill_report.md)), a project now records how fine its source data is so a generated dataset stores the shape it can be rebuilt at, and a save can no longer move a hand-entered dataset's stored shape or orphan its file. |
