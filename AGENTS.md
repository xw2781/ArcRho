...
## ArcRho Orchestrator Deployment Authorization
The user pre-authorizes agents to stop running ArcRho Orchestrator instances and to rebuild, redeploy, and relaunch the ArcRho Orchestrator whenever a task changes code the Orchestrator bundles or requires newly implemented orchestration behavior to become active. Do not stop ArcRho Engine, the Bridge, ResQ, or other workloads under this authorization; stopping the Save Gateway is covered by its own authorization below. Those processes keep running while Orchestrator replenishment is briefly unavailable. Continue to request any platform-required sandbox escalation, but do not request separate conversational confirmation for the Orchestrator stop, rebuild, deploy, or relaunch.

Use `data-engine/src/arcrho_orchestrator/build_exe.py` as the canonical workflow and inspect its imports, PyInstaller paths, and deployment logic before deciding whether a source change is bundled. Run it with Python 3.10. The script builds into an isolated staging folder, sets `apps.orchestrator.kill_all`, waits for live Orchestrator heartbeats to disappear, swaps the deployed app folder atomically with rollback, restores the prior kill-switch value, and starts a deployed Orchestrator when none is live. Do not substitute manual process termination or an in-place executable overwrite for this workflow.

After deployment, verify a fresh Orchestrator heartbeat under `E:\ArcRho Server\runtime\instances\arcrho_orchestrator\` and confirm the deployed process is running. When the task changes Orchestrator supervision of another component, also verify that component's fresh heartbeat or other canonical readiness signal. A deploy relaunches at least one Orchestrator; other Windows users' per-session Orchestrators may return through their normal Launcher/login lifecycle, while their already-running Engine, Bridge, and Gateway processes remain untouched.

Do not rebuild for changes limited to Orchestrator tests, docs, release fragments, or unrelated files. Never deploy a change that has not passed its checks. If the build fails, the live Orchestrator does not stop within the script's timeout, the atomic swap rolls back, or the relaunched heartbeat does not become fresh, report the outcome plainly and do not retry blindly. State in the final response whether the Orchestrator was rebuilt, redeployed, and relaunched, or why it was not.
...
## ArcRho Save Gateway Deployment Authorization
The user pre-authorizes agents to stop the running ArcRho Save Gateway and to rebuild, redeploy, and relaunch it whenever a task changes code the Gateway bundles. Do not stop ArcRho Engine, the Bridge, the Orchestrator, ResQ, or other workloads under this authorization. Continue to request any platform-required sandbox escalation, but do not request separate conversational confirmation for the Gateway stop, rebuild, deploy, or relaunch.

The Gateway ships a frozen copy of its bundled sources, so a change to those sources has no effect on the running Gateway until it is rebuilt. `data-engine/src/arcrho_save_gateway/build_exe.py` owns the recipe; read its `--paths` and `--hidden-import` list rather than trusting a copy. At the time of writing it bundles `data-engine/src/arcrho_save_gateway/`, `data-engine/src/`, and `python-api/src/`, so a change to a shared contract such as `arcrho_hosted_save_http_contract` or `arcrho_engine_save_contract` still requires a rebuild.

Run `python data-engine/src/arcrho_save_gateway/build_exe.py` with a Python 3.10 interpreter. That single command builds, stops the live Gateway through `apps.save_gateway.kill_all`, waits for its heartbeat to disappear, swaps the deployed app folder atomically with rollback, restores the prior kill-switch value, and relaunches the Gateway when none is live. This command may run to completion without asking.

The build may also run from a Client PC against the server workspace over the network, so the Gateway does not have to be rebuilt at the Dev PC. Set `ARCRHO_DEPLOY_ROOT` to the mapped or UNC ArcRho Server path; the script aligns the workspace root from it before reading any configuration and refuses a pair that names two different workspaces, so the kill switch, heartbeats, and deployed folder always belong to the same server. When the deploy root is not a local fixed disk the script does not start the deployed executable, because that would run the server's Gateway on the build machine; clearing the kill switch is enough, and the server's Orchestrator restores it. Verifying the deploy then means checking the server's heartbeat folder and `GET <client_url>/api/health` as usual.

Stopping the Gateway makes hosted saves fail for enrolled users until it returns, and a save in flight during the swap must be retried, so rebuild once per task, after the change is verified and before reporting the task complete. Skip the rebuild when a task changed only Gateway tests, docs, release fragments, or files outside the bundle, and say so. After deployment, verify a fresh heartbeat under `E:\ArcRho Server\runtime\instances\arcrho_save_gateway\` and confirm `GET <client_url>/api/health` returns `{"ok": true}`. Never deploy a change that has not passed its checks. If the build fails, the live Gateway does not stop within the script's timeout, or the swap rolls back, report the outcome plainly and do not retry blindly. State in the final response whether the Gateway was rebuilt, redeployed, and relaunched, or why it was not.
