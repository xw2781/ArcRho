...
## ArcRho Gateway Deployment Authorization
The user pre-authorizes agents to stop the running ArcRho Gateway and to rebuild, redeploy, and relaunch it whenever a task changes code the Gateway bundles. Do not stop ArcRho Engine, the Bridge, the Orchestrator, ResQ, or other workloads under this authorization. Continue to request any platform-required sandbox escalation, but do not request separate conversational confirmation for the Gateway stop, rebuild, deploy, or relaunch.

The Gateway ships a frozen copy of its bundled sources, so a change to those sources has no effect on the running Gateway until it is rebuilt. `data-engine/src/arcrho_gateway/build_exe.py` owns the recipe; read its `--paths` and `--hidden-import` list rather than trusting a copy. At the time of writing it bundles `data-engine/src/arcrho_gateway/`, `data-engine/src/`, and `python-api/src/`, so a change to a shared contract such as `arcrho_hosted_save_http_contract` or `arcrho_engine_save_contract` still requires a rebuild.

Run `python data-engine/src/arcrho_gateway/build_exe.py` with a Python 3.10 interpreter. That single command builds, stops the live Gateway through `apps.gateway.kill_all`, waits for its heartbeat to disappear, swaps the deployed app folder atomically with rollback, restores the prior kill-switch value, and relaunches the Gateway when none is live. This command may run to completion without asking.
...
Stopping the Gateway makes hosted saves fail for enrolled users until it returns, and a save in flight during the swap must be retried, so rebuild once per task, after the change is verified and before reporting the task complete. Skip the rebuild when a task changed only Gateway tests, docs, release fragments, or files outside the bundle, and say so. After deployment, verify a fresh heartbeat under `E:\ArcRho Server\runtime\instances\arcrho_gateway\` and confirm `GET <client_url>/api/health` returns `{"ok": true}`. Never deploy a change that has not passed its checks. If the build fails, the live Gateway does not stop within the script's timeout, or the swap rolls back, report the outcome plainly and do not retry blindly. State in the final response whether the Gateway was rebuilt, redeployed, and relaunched, or why it was not.
