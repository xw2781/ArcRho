...
The user pre-authorizes agents to stop running ArcRho Orchestrator instances and to rebuild, redeploy, and relaunch the ArcRho Orchestrator whenever a task changes code the Orchestrator bundles or requires newly implemented orchestration behavior to become active. Do not stop ArcRho Engine, the Bridge, the Save Gateway, ResQ, or other workloads under this authorization. Those processes keep running while Orchestrator replenishment is briefly unavailable. Continue to request any platform-required sandbox escalation, but do not request separate conversational confirmation for the Orchestrator stop, rebuild, deploy, or relaunch.
