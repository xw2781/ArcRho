...
- Table wrap framing should keep one visible outer edge across the table body, sticky header, total row, and scrollbar area. Main option: treat the scroll wrapper and scrollbar gutter as part of the frame; keep the wrapper border, reserve stable scrollbar gutter space, add minimal right/bottom breathing room when needed, and preserve the last visible row/column borders, including masked blank cells, so the grid edge remains visible at max scroll. Alternative solution: when cells already draw the full grid edge and the scrollbar area cannot hide it, keep the wrapper border visually neutral to avoid doubled outlines.
- Scrollable table wrappers should use pale neutral track/corner fill when the scrollbar area is part of the table frame, with restrained thumbs that are quiet at rest and stronger only while scrolling or when the pointer is over the scrollbar lane, not on whole-table hover.
