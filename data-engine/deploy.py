...
it usable from a script or an agent rather than only by hand.
    ...
    python data-engine/deploy.py                 # every stale component
    python data-engine/deploy.py bridge engine   # named components
    python data-engine/deploy.py --stale         # report only, build nothing
    python data-engine/deploy.py --ref main      # build a pushed ref, no patch
