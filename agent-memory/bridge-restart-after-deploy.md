...
**Other users' sessions are collateral.** A `server-components/deploy.py bridge` run kills every live Bridge on the server (`apps.bridge.kill_all` flips true then false), but only the deploying user's session gets a worker back. On 2026-08-25 a deploy left JZhang's `arcrho_bridge_worker` heartbeat gone while xwei's reappeared within 20s. Check the worker folder for the other users you saw before the deploy, and tell them to relaunch their ArcRho session — nothing on the server will do it for them.
