...
Whether the Bridge comes back after `data-engine/src/arcrho_bridge/build_exe.py` depends on `apps.bridge.auto_create_instance` in `E:\ArcRho Server\config\config.json`. It was false on 2026-08-04 but true again on 2026-08-07 — check the live value, don't assume.

**Why:** The orchestrator loop only spawns Bridge instances when `auto_create_instance` is true (`data-engine/src/arcrho_orchestrator/main.py`). Since 2026-08-07 the model is one Bridge per user session: the orchestrator counts/limits only its own user's bridge heartbeats, and the Bridge supervisor exits at startup if a fresh same-user heartbeat exists.
