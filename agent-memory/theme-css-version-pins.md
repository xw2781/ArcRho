...
`frontend/tests/color_theme.test.mjs` pins exact `?v=` stamps for the shared theme sheets: the `THEMED_DOCUMENTS` ordering test pins `color_theme.js`, `light.css`, `dark.css` and `revolutionary.css` stamps (one shared stamp each across all runtime documents; High Contrast was removed 2026-09-23 and `revolutionary.css` took its slot as the last theme sheet), the splash test pins splash.html's `light/dark` stamps, the DFM dark test pins dfm.html's `dark.css?v=`, and the "cache-version chains" test pins page-entry module stamps (e.g. `dataset_viewer_main.js?v=`).
