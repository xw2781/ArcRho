...
The Client PC clone has full local component venvs under `server-components\venvs\` (engine venv = Python 3.10.11 with pandas, pywin32, fastapi, pydantic, numpy, openpyxl). `.vscode/settings.json` (gitignored, machine-local) points the Python interpreter and Code Runner at `server-components\venvs\arcrho_engine\Scripts\python.exe`, so VS Code "Run" uses 3.10 instead of the bare `python` (3.13, no packages).
...
**How to apply:** For a real migration run, execute on the Server PC (NE7SASWPN02), e.g. `E:\XWSpace\Repos\ArcRho\server-components\venvs\arcrho_engine\Scripts\python.exe -u ...\python-api\migration\resq_data_migration.py`. Its `main()` performs no writes before the ResQ connect succeeds, so a dry attempt on the Client PC is harmless. That clone is the Build Listener's repository ([[remote-component-deploy]]) — it gets hard-reset by deploys, so check its checked-out state first.
