...
- [Gateway deploy swap lock](gateway-deploy-swap-lock.md) — the atomic folder swap can fail with WinError 5; rollback keeps the old Gateway healthy, and the wrapper exit code lies, so read the output tail
