...
- [Engine calculation gateway transport](engine-calculation-gateway-transport.md) — Phase 2 live 2026-08-16: ArcRhoTri/Vec/Headers publish-and-wait runs on the Gateway (~300 ms vs ~1.2 s per exchange); transport decides on is_network_path(data_path) so tests stay local; safe live-probe recipe
