...
**How to apply:** when a deploy fails with exactly this message and an otherwise empty build log, rerun `deploy.py` once. If it happens twice, the fix belongs in `data-engine/src/arcrho_build_listener.py` `_read_request` (retry a `PermissionError` for a second or two). Related: [[remote-component-deploy]].
