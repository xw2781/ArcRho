...
metadata: 
  ...
  modified: 2026-09-21T17:43:20.914Z
---
...
**How to apply:** Treat any change to `_calculate_vectors` (BF) or the Cape Cod equivalent as a persisted-data migration: ship the contract change together with a republish of every affected method file (method JSON, output sidecar with a new publication revision, output CSV) and a dependent walk, or with an approved tolerance in the load path. Ask the user which before deploying. The user chose the republish: `tools/restate_bf_unobserved_origin_ultimates.py --project X --apply` recalculates each stale BF from its embedded snapshots, publishes through the service's `_publish`, and queues one walk per class with every restated output as a root (deploy the new contract first). Run per project: the auto-mode classifier refuses `--all-projects`, even as a dry run. Related: [[hosted-save-fix-needs-engine-deploy]], [[deploy-only-when-released-app-is-safe]], [[bulk-method-restatement-hold]].
