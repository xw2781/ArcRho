...
- The app the user is running is the packaged build from `%LOCALAPPDATA%\Programs\ArcRho\resources`, not the working tree. Diff `resources\arcrho_server\_internal\ui\...` against `frontend/ui/...` before assuming the running code is the code you are reading; `server-components/deploy.py` does not rebuild it, only Bridge/Engine/Gateway/Orchestrator/Admin/Launcher.
