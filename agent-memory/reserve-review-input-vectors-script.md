...
metadata: 
  ...
  modified: 2026-09-01T00:23:32.583Z
---
...
**How to apply:** do not assume one vector set per class serves every method in it. A consumer must check that the row a relative index resolves to is one of the method's own origin labels. If an annual companion series is ever wanted inside a quarterly class, a second instance of the same dataset type under a different name is supported — 12 such pairs already exist in that project, e.g. two instances of type `F 00 - Ultimate Net Loss`. It carries a cost worth pricing in first: a method references a dataset by name alone, so two instances of one type cannot be told apart by the reference, and every consumer would need a rule for which instance serves which method's step. The user has not decided whether to build them. See [[mixed-origin-length-precedents]] for the refresh-failure face of the same root cause.
