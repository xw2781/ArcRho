...
metadata: 
  ...
  modified: 2026-08-16T15:22:01.907Z
---
...
Likely cause is a lock race, not permissions: the server's Orchestrator restarts the Gateway as soon as the kill switch clears, so the deployed folder can be reopened between the stop and the rename. Deploying from the Client PC normally works (see [[client-pc-primary-workstation]]), so treat this as retryable rather than a blocker — but AGENTS.md says report a failed deploy plainly and do not retry blindly.
