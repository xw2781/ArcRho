...
// wants to know: the source table, not a hand-entered grid or a formula. The
// Engine fills one of those two ways, so it owns two labels: a dataset bound
// to a source column is Imported, and one the Engine evaluates as a formula
// over other dataset types is Generated.
const KIND_LABEL_BY_FAMILY = Object.freeze({
  dataset: "Dataset",
 ...
 * it is entered, calculated by a formula, imported from the source table, or
 * generated by the Engine from a formula over other types - which is what the
 * server's hydrated formula on an engine node says. A name a sidecar still
 * references but the index no longer lists is its own family, so a dangling
 * edge reads as a problem rather than as a dataset.
 */
export function dependencyNodeKind(node = {}) {
  ...
  if (sourceKind === "engine") {
    const family = text(node.formula) ? "generated" : "engine";
    return { family, label: KIND_LABEL_BY_FAMILY[family] };
  }
  if (sourceKind === "calculated") {
    return { family: "calculated", label: KIND_LABEL_BY_FAMILY.calculated };
  }
  return { family: "dataset", label: KIND_LABEL_BY_FAMILY.dataset };
}
