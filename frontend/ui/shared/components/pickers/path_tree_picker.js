...
function ensureSharedScrollbarStyles(doc) {
  ...
  const existing = Array.from(doc.querySelectorAll('link[rel="stylesheet"]')).some((link) =>
    String(link.getAttribute("href") || "").includes("/ui/shared/styles/scrollbars.css"),
  );
  if (existing) return;
  const link = doc.createElement("link");
  link.id = SHARED_SCROLLBAR_STYLE_ID;
  link.rel = "stylesheet";
  link.href = "/ui/shared/styles/scrollbars.css?v=20260715a";
...
export function openFloatingPathTreePicker(options = {}) {
  ...
  const renderFavoriteSection = () => {
    ...
    if (typeof options?.onShortcutConfigMenu === "function") {
      favoriteHeader.header.addEventListener("contextmenu", (evt) => {
        evt.preventDefault();
        evt.stopPropagation();
        options.onShortcutConfigMenu({ event: evt, headerElement: favoriteHeader.header, pickerElement: win });
      });
    }
  ...
  // A settings file dropped anywhere on the panel loads it.
  if (typeof options?.onDropConfigFile === "function") {
    attachSettingsFileDropZone(win, {
      name: "path-tree-config",
      hint: options?.configDropHint,
      documentRef: doc,
      onFile: (file, ctx) => options.onDropConfigFile(file, { ...ctx, pickerElement: win }),
    });
  }
