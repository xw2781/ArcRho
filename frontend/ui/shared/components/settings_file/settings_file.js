...
function ensureStyles(doc) {
  ...
  link.id = STYLE_ID;
  link.rel = "stylesheet";
  link.href = "/ui/shared/components/settings_file/settings_file.css?v=20260920a";
...
/**
 * Asks for a settings file through the desktop host's open dialog and returns its parsed
 * payload. Resolves to `{ ok: true, data, path }`, or `{ ok: false, reason }` where reason
 * is `no-host`, `canceled`, or `error` with an `error` message meant for the status bar.
 */
export async function openSettingsFile({ kind, label = "settings", filterName = "ArcRho Settings", startDir = "" } = {}) {
  const host = window.ADAHost || window.top?.ADAHost || null;
  if (typeof host?.pickOpenFile !== "function" || typeof host?.readTextFile !== "function") {
...
/**
 * Opens the panel's settings menu at the pointer: "Save <label>" and "Load <label>".
 * `label` names what the file holds, so one panel's menu can never be mistaken for
 * another's.
 */
export function openSettingsFileMenu({ event, label = "Config", onSave, onLoad, documentRef } = {}) {
