...
===============================================================================
*/
import { buildDatasetExternalLinkTargets } from "/ui/shared/dataset/dataset_external_links.js?v=20260829a";
import {
...
export function createDatasetInternalLinksController({
  ...
  function getCellLinkInfo(displayRow, displayColumn) {
    ...
    const decoration = getTargetDecorationIndex().get(targetCellKey(actual));
  ...
  function decorateCell(cell, displayRow, displayColumn) {
    ...
    const link = decoration?.link;
    const isRange = !!decoration?.isRange;
    const failure = failuresByTargetKey.get(key);
    cell.classList.toggle("arInternalLinkCell", !!link);
    cell.classList.toggle("arInternalLinkErrorCell", !!failure);
    if (link) {
      cell.classList.toggle("arArrayFormulaCell", isRange);
      cell.classList.toggle("arArrayFormulaEdgeTop", isRange && decoration.edgeTop);
      cell.classList.toggle("arArrayFormulaEdgeRight", isRange && decoration.edgeRight);
      cell.classList.toggle("arArrayFormulaEdgeBottom", isRange && decoration.edgeBottom);
      cell.classList.toggle("arArrayFormulaEdgeLeft", isRange && decoration.edgeLeft);
