(function () {
  ...
  const STORAGE_PREFIX = "arcode";
  const ZOOM_STORAGE_KEY = `${STORAGE_PREFIX}_ui_zoom_pct`;
  const ZOOM_MODE_KEY = `${STORAGE_PREFIX}_zoom_mode`;
  const STATUSBAR_H_KEY = `${STORAGE_PREFIX}_statusbar_h`;
  const WIRE_KEY = "__arcodePageZoomBridgeState";
  const FORWARD_TYPES = new Set(["arcode:zoom", "arcode:zoom-step", "arcode:zoom-reset"]);
  ...
  function notifyApplied(state, detail) {
    window.dispatchEvent(new CustomEvent("arcode:page-zoom-applied", { detail }));
  ...
  function wirePageZoomBridge(options = {}) {
    ...
    if (!state.wired) {
      ...
      document.addEventListener("wheel", (event) => {
        ...
        relayToParent({ type: "arcode:zoom", deltaY: event.deltaY });
  ...
  window.ArcodeZoomBridge = bridge;
})();
