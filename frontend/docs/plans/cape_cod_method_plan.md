...
| Prior Ultimate (+ Type dropdown) | `method_tab.prior_ultimate_dataset`, `method_tab.prior_ultimate_mode` | `latest_ultimates` (ResQ `pdInternal=0`): a prior ultimate Vector (typically a DFM output); percentage developed is `Latest / Prior Ultimate`. `pattern` (ResQ `pdPattern=1`): the selected vector holds percentage-developed values directly. ResQ's DFM-factor types `pdCumDevFactors=2` and `pdCumDevFactorsAdjusted=3` import as `latest_ultimates`: the referenced DFM vector's `Latest / Ultimate` ratio equals ResQ's percentage developed exactly; when that DFM output is empty (ResQ then reports a flat 1.0 pattern) the importer snapshots `Latest / PercentageDevelopedValues` as the effective prior ultimates instead of the stored zeros. |
