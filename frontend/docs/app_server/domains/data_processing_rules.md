...
- Dataset-scoped source values are derived lazily into `projects/<project>/data_processing_values.json` with format `arcrho-source-vocab-v1`. The cache stores complete, de-duplicated reserving-class key combinations for each mapped source measure; it does not change the persisted rules contract.
...
- `json_format`, `revision`, `updated_at`, and `updated_by` are server managed. Revision increments only when the semantic rules change.
- Saves use a project-path lock, unique temporary file, and atomic replacement. Audit entries summarize added, edited, removed, enabled, and disabled rules.
