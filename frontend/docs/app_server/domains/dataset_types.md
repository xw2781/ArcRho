...
| `POST` | `/dataset_types` | `save_dataset_types` | `DatasetTypesSaveRequest` | [`app_server/schemas/dataset_types.py`](../../../app_server/schemas/dataset_types.py) | `calculated_dataset_service.changed_formula_dataset_type_names`, `dataset_types_change_service.submit_dataset_types_change_job`, `dataset_types_service.change_needs_project_job`, `dataset_types_service.normalize_submitted_rows`, `dataset_types_service.read_persisted_rows`, `dataset_types_service.require_resolvable_formulas`, `dataset_types_service.resolve_persisted_rows` |
...
- `POST /dataset_types` never performs the long rebuild inside the request. It answers with `applied: "direct"` for a change that re-derives nothing outside the table, or `applied: "job"` plus `job.job_id` for one that does; `GET /dataset_types/change_job/status?project_name=&job_id=` then reports that job's `status`, `progress`, `result` and whether the project is still `busy`.
...
- A change is classified before anything is written. It needs the project-wide job when a type disappeared, when a surviving type's `Data Format`, `Calculated`, `Formula` or `Generated` moved, or when the components a calculated formula resolves to changed. Everything else -- Category, row order, and **adding an ordinary dataset type** -- is written directly by the app server, because a new type has no instances and no sidecar naming it.
...
- Removing a dataset type is refused, before the table is written, while any instance of it is still read by a dataset or method that is not itself being removed (`calculated_dataset_service.find_dataset_type_removal_blockers`). A calculated formula naming a removed type is already refused earlier by `require_resolvable_formulas`; this covers the other half, the instances and their downstream readers. An instance nothing reads is not a blocker, so an unused type can always go.
- The project-wide job is Engine-hosted under `arcrho_dataset_types_change_contract`. Submission preflights a live Engine, that no project-scope job already runs, that no reserving class of the project is still being walked, and that no source refresh holds the project; it then publishes a queued status and the request and returns. ArcRho Engine claims the project-scope propagation lease, writes `dataset_types.json` and `dataset_types.xlsx` through the same `dataset_types_service.apply_dataset_types_rows` the direct path uses, appends the audit entry, then re-derives every sidecar's `formula`, `Calculated`, `Precedents` and `Dependents`, recalculates the changed calculated dataset types, and walks each affected class's dependents.
- While that lease is held, `find_reserving_class_propagation_hold` reports `{"reason": "project"}` for every class of the project, so every save meets the `423` it already handles and no propagation or source-refresh job can be queued. A partially failed rebuild is a terminal `error` whose message says the table was saved, because the table is already the new one.
- The job's `progress` is counted in reserving classes while it scans the project (stage `scanning`, one unit per class, which is the only denominator known before the work starts), then in dataset instances (one unit each), plus one unit for the table write and one per reserving class the change makes the walk revisit. The rebuild's total is unknown until it has listed the project, so the denominator grows once at that point; the completed count never decreases. Its `result` reports `datasets_total`, `datasets_updated`, `classes_total`, `classes_walked` and `datasets_recalculated`.
