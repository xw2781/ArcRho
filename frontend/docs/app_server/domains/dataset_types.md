...
- Which types a formula reads is decided once, by `arcrho_api.dataset_type_contract.formula_references`: every double-quoted name first, then every unquoted name the table already holds, longest first with each match taken off the text so `Premium` is not also read out of `Earned Premium`. Source expansion, save validation and the persisted dependency links all call it, so a formula that quotes one input and leaves another unquoted -- only a hand-edited formula does, since ResQ quotes every name -- counts both wherever it is read. `dataset_type_formula_graph` and `formula_closure` beside it answer the same question for a whole table, including the generated types the Engine computes, whose formulas name other types just as an app-calculated one does. `generated_formula_refresh_names` is the one built on top of them: given the dataset types a source refresh was narrowed to, it returns those plus every type the Engine builds from a formula that reads one of them, which is what the Engine's Import Data and Data Processing Rules jobs rebuild.
<!-- MANUAL:END -->
