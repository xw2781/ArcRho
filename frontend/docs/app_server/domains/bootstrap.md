...
- The output sidecar owns Notes, Audit Log, status, `Precedents` (the DFM's output dataset and the target ultimate vector), and `Dependents`, and carries `source_kind: "bootstrap"`, `method_type: "Bootstrap"`, `method_type_code: 6`, `data_format: "Vector"`. The published vector is the scaled expected ultimate at full floating precision, with aggregated 3/6/12 variants. The reserving-class `index.json` remains a minimal scalar inventory.
<!-- MANUAL:END -->
