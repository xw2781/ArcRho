...
- Automatic server refresh preserves exclusions, formula definitions and selections, literal User Entry values, the complete stored result of Excel- and dataset-linked formulas, ratio-cell notes, Method Notes, Audit, and the method-owned origin/development axes. Source CSV values are mapped onto those axes without requiring duplicate precedent-sidecar labels. A change to a dataset used only by a Ratios formula follows its registered graph edge and marks the DFM Review Needed; opening the DFM resolves that formula in memory and requires an explicit Save to publish it. Refresh alone is not acknowledgement. Incompatible row, column, or period geometry leaves the prior publication intact and reports the refresh failure separately.
