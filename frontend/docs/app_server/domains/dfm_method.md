...
- Save compares the owned revision independently from the derived revision. A clean owned patch may rebase over an automatic derived refresh; an externally changed owned revision returns a conflict.
- Publication uses the reserving-class lock, staged replacement, rollback, unchanged-file suppression, and sidecar-last ordering. Ratio-Basis-only refreshes do not rewrite output CSVs or cascade when the ultimate vector is unchanged.
- Automatic refresh preserves exclusions, formula definitions and selections, literal User Entry values, the complete stored result of Excel-linked formulas, ratio-cell notes, Method Notes, and Audit. Incompatible development geometry leaves the prior publication intact and marks the method Review Needed.
<!-- MANUAL:END -->
