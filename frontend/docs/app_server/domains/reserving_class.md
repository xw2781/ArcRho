...
| `GET` | `/reserving_class_paths_with_data` | `get_reserving_class_paths_with_data` | `str` | - | `reserving_class_service.list_reserving_classes_with_data`, `workspace_read_client.run_workspace_read` |
...
- These reads are on the Project Instance page-load critical path and are not registered workspace-read kinds, so they always run against the mapped drive, where a file read costs a full round trip regardless of payload size. Each request therefore reads each file once: `GET /reserving_class_filter_spec` takes one preference-file read for both the filter spec and the tree preferences, and `refresh_reserving_class_types_json` takes one read of `reserving_class_types.json` that serves both the merge base and the unchanged-file comparison that decides whether to write. `refresh_reserving_class_types_json` still merges newly discovered source values and still writes the paired JSON/XLSX when the merge differs; only the redundant reads were removed. `tests/test_reserving_class_read_io.py` pins those read counts alongside the write behaviour.
<!-- MANUAL:END -->
