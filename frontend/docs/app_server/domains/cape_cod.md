...
- Managed source saves follow registered reverse edges and refresh only affected Cape Cod source snapshots via the shared refresh waves (`include_cape_cod` guard prevents recursion). Refreshed outputs apply the shared content-based review policy: unchanged method JSON/output CSVs preserve green, meaningful changes require review, and existing review alerts remain until explicit Save.
<!-- MANUAL:END -->
...
- A precedent is read at its stored period, not its displayed one, and a precedent stored finer than the method's `origin_length` is brought to it through `precedent_cache_service.precedent_source`: an Engine-generated Exposure or Prior Ultimate vector is rebuilt at the method's period, a hand-entered one is rolled up in memory from its own CSV, and only a coarser precedent is refused with `422 … uses N-month origins; expected M`.
...
- The output sidecar owns Notes, Audit Log, status, `Precedents` (Latest, Exposure, Prior Ultimate), and `Dependents`. Every successful automatic refresh stamps the sidecar's `updated_at`/`modified_by` and appends an `Auto Refresh` audit record even when the published output is unchanged; output CSVs are republished regardless of whether the publication values change. The reserving-class `index.json` remains a minimal scalar inventory.
