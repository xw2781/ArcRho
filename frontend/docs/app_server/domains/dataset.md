...
- Exposes dataset sidecar load/save endpoints. Dataset sidecars live in `projects/<project>/data/<ReservingClassFolder>/sidecars/<DatasetName>.json`; `dataset_name` is the persisted user-visible instance name, `dataset_type` is the persisted Dataset Type, and the old root aliases `instance_name` and `dataset_type_name` are not accepted. Save updates metadata, preserves caller-supplied `source_kind`/`data_format`, applies current formula graph fields, refreshes the selected reserving-class `index.json`, and returns dependent recalculation details when the saved dataset feeds app-calculated outputs.
