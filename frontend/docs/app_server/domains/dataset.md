...
- Exposes cached empty dataset creation for Project Instance non-generated dataset types. The endpoint writes a zero-filled generated-data CSV and base metadata sidecar for the selected project/reserving-class/dataset type, using the existing length/mode-scoped filename convention and deriving the actual triangle/vector shape from the project's `general_settings.json` date boundaries.
...
- ArcRhoTri CSV request targets are `projects/<project>/data/generated/<ReservingClassFolder>/<InstanceName>@<OriginLength>@<DevelopmentLength>@<cum|inc>@<dev|cal>.csv` when `InstanceName` is supplied, otherwise the legacy Dataset Name is used. The metadata sidecar is the base `<InstanceName>.json` file and records the generated instance name separately from `dataset_type`, plus `data_format`, saved `origin_length`, `development_length`, `cumulative`, `calendar`, `created`, `updated_at`, and user fields. The reserving-class path is a single filename-escaped folder name using the reversible `_%XX_` rule and is not repeated in the CSV filename.
