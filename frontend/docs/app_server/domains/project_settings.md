...
## Data/State/Caches
<!-- MANUAL:BEGIN -->
- Handles project-index CRUD and settings JSON writes. Project names and virtual UI folders live in `projects/index.json`; source table paths live in each project's `field_mapping.json` and are never touched by registry reads or writes.
- `GET /project_settings/{source}` serves the registry as `folders` plus `project_paths` (each path's leaf segment is the project name) with the index `mtime` - one index read, no per-project file access. `POST /project_settings/{source}` is the authoritative registry write: `project_paths` is the complete project list (projects absent from it are removed), `folders` adds empty virtual folders, and `file_mtime` keeps the optimistic concurrency check. `GET /project_settings/{source}/folders` remains a read-only registry view for shared pickers. The write publishes a temp file over `index.json`; because Windows refuses that swap while any other handle (even a reader) is open, the swap is retried five times over about two seconds before the route reports `423`, and the duplicate-project finalizer retries a `423` twice more before surfacing it.
...
- `POST /project_settings/{source}/duplicate_project_folder/cancel/{request_id}` asks the Engine to stop a queued or running duplicate by publishing a location-independent cancel marker under `<server-root>/requests/project_duplication/cancel/`. The Engine reads the marker between copy steps and, when it finds it before the staging folder is renamed into place, discards the staging folder and publishes a terminal `cancelled` status; a job that already reached `success`, `error`, or `cancelled` is reported as-is with `cancel_requested: false`, and the marker is cleared with the rest of the job's durable metadata.
