...
- `GET /project_settings/{source}` serves the registry as `folders` plus `project_paths` (each path's leaf segment is the project name) with the index `mtime` - one index read, no per-project file access. `POST /project_settings/{source}` is the authoritative registry write: `project_paths` is the complete project list (projects absent from it are removed), `folders` adds empty virtual folders, and `file_mtime` keeps the optimistic concurrency check. `GET /project_settings/{source}/folders` remains a read-only registry view for shared pickers.
