...
## Known Risks
<!-- MANUAL:BEGIN -->
- A propagation refresh writes `method_metadata.data_refreshed` and leaves `last_modified` -- the last user Save -- untouched, matching DFM, BF, CC and bootstrap. The ResQ reserving-class sync reads `last_modified` as the ArcRho edit time, so a method recomputed because an input changed is not offered as a change to push.
- `method_revision` covers the method's content and deliberately excludes `method_metadata.last_modified` and `data_refreshed`, matching DFM, BF, CC and bootstrap, whose revisions come from `dfm_contract.method_revisions` projections that never reach them. That is what lets an RPC upload record ResQ's save time without invalidating the token an open editor saves with. A revision is minted where the method loads (a Client PC's app server, or the Gateway) and checked where it saves (always ArcRho Engine), and those upgrade separately, so `save_result_selection` also accepts `_legacy_method_revision` -- the whole-file hash minted before the exclusion. Remove that fallback once every installed app carries the change.
