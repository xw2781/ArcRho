...
- A save runs in two steps. Every save endpoint has a sibling `<save path>/plan` (`Mode: "plan"` in the same hosted-save contract) that runs `save_plan_service.build_save_plan` on the Engine: it resolves the save's propagation roots through `save_propagation_roots` in the module that owns the save, unions the sidecar `Dependents` closure with the dataset-type formula closure, and answers with the reachable objects plus a `fingerprint` of everything it read. It runs no save, changes no saved object, and — deliberately — takes no reserving-class lease, because holding one across the human pause would block every other save in the class for as long as the dialog stayed open. The page shows `ui/shared/services/save_plan.js`'s confirmation and, only on approval, sends the save with `plan_fingerprint`; the Engine recomputes the plan *under* the lease and refuses with 409 (`SAVE_PLAN_STALE_MESSAGE`) when the class moved in between. A plan reaching nothing skips the dialog. The list is a superset by design: only the walk can tell whether an intermediate method output actually changed.
...
3. Add a hosted-save kind: register it in `SAVE_JOB_KINDS`, give its service a `save_propagation_roots` that mirrors how the save derives its roots (the plan resolves them through that one function, so the two cannot drift), add the `/plan` route beside the save route with one shared argument projection, and call `planAndConfirmSave` in the page before its first write — for a window that writes files of its own, before those writes too, not just before the sidecar save.
<!-- MANUAL:END -->
...
- The two-step save costs one extra Engine round trip per save (~1-2 s: the poll interval plus the SMB status reads). It is spent even when the plan turns out to reach nothing, because only the plan can tell.
- A refused plan blocks its save rather than falling back to saving unconfirmed: the plan preflights the same conditions the save does (503 no Engine, 423 busy class), so silently proceeding would drop the review step exactly when the class is least predictable. The unsaved edit stays in the editor.
- The staleness recheck is a plain fingerprint comparison, not a merge: a save refused with 409 is simply re-planned by the user's next Save. Deliberate, and consistent with the existing `expected_revision` conflicts.
<!-- MANUAL:END -->
