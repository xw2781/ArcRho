...
- Each hosted save or plan appends one structured client-side timing record to `%LOCALAPPDATA%\ArcRho\logs\client_save_latency.jsonl` (`Arcode` replaces `ArcRho` in Arcode mode). The local JSONL log rotates at 5 MB with three backups and records the logical project/class/object labels, outcome, total duration, workspace/path/heartbeat/hold/request/status timings, status-poll observations, and whether the response came from the terminal status or legacy result file. It does not record request payloads or method data. `remote_round_trip_ms` intentionally treats Engine execution as one opaque interval; the log is for diagnosing client-to-network-drive latency. Terminal artifact cleanup is correlated by request id in a separate record because it runs off the response path. Logging is best-effort and cannot fail a save.
