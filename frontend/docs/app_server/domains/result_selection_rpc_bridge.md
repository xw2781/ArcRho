...
## External Interfaces
<!-- MANUAL:BEGIN -->
- `SyncResultSelection` status JSON carries `last modified`, the `Modified` ResQ stamped when the confirmed write-back saved. After a successful upload the app server records it as the local method's `method_metadata.last_modified` through `result_selection_service.record_rpc_sync_last_modified`, and reports the outcome under `last_modified_record`. Without it the two copies hold identical settings under different times and the next comparison ranks the remote newer, offering to pull back what was just pushed; ResQ's own value is used rather than this machine's clock because the comparison matches on the instant. A Bridge that does not report it, a missing method, a legacy v1 file, or a propagation walk holding the class all leave the local timestamp untouched -- the upload has already succeeded, so a failure to record is reported beside it rather than turned into a failed sync.
- Transport: the same split the DFM RPC bridge uses. `compare` is the `result_selection_rpc_bridge_compare` workspace read and `sync`, `keep-local`, `cleanup`, and `update-remote` are `result_selection_rpc_bridge_*` workspace mutations, hosted on the ArcRho Gateway when it advertises the kind and run over the mapped drive otherwise. `apply` stays local. The request-file contract and the Bridge are unchanged; `UserName` names the acting user.
...
- `arcrho_bridge` handles `Function = ResultSelection` requests by smart-matching the requested ArcRho method/output name to ResQ Result Selection names without renaming ResQ instances, then exporting the canonical v2 Method payload into grouped `RS@<Name>.json`. It uses ResQ `OriginCount` for row count and includes source metadata/native origin length, values, nonnegative weights, named Ratio Basis values, derived `calculated_ultimate`, explicit `ultimate_overrides`, final `selected_ultimate`, notes, and `method_metadata.last_modified`; persisted numeric arrays use symmetric half-away-from-zero rounding to six decimal places. It handles confirmed `Function = SyncResultSelection` requests by reading `MethodJsonPath`, writing matching source weights from `loaded_datasets` with `SetWeights`, clearing and reapplying only explicit ultimate overrides with `SetUltimates`, updating notes and Origin Length, calling `Save()`, and writing the status JSON to `DataPath`.
...
- Applying the remote version first validates the complete v2 payload and requested method identity, then delegates to the central transactional Result Selection save using the current local revision. Method JSON, output CSV variants, sidecar, reverse dependency edges, and downstream refresh therefore succeed together before the remote temporary JSON is deleted. The frontend applies the already-saved response without issuing a second non-transactional save.
