...
- `ui.reviewTableOpen` returns a `dialogId` immediately, so the automation queue remains available while a user searches, selects, and reviews typed rows. `ui.reviewTableStatus` returns `pending` until review finishes, then returns `{ accepted, selectedRowIds }`; `ui.reviewTableClose` cancels an unfinished review and removes its transient shell state. Row IDs must be stable and unique, and disabled rows are never returned as selected actions.
...
- Review-table rows and completion state live only in shell memory for the returned `dialogId`; callers close the review after consuming its completed status.
