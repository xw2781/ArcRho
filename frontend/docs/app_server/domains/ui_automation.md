...
- A read-only table that names its own `cancelLabel` gets a Cancel button beside the accept one, which is how a report becomes a confirmation: nothing to tick, but two ways out, and `accepted` says which was chosen. The Export Reserving Class to ResQ macro shows its ArcRho/ResQ timestamp comparison this way before it writes anything. Without a `cancelLabel` a read-only table keeps its single closing button.
