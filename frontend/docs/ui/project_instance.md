...
- Project Instance watches the selected reserving-class `data/<ReservingClassFolder>` cache folder, including its `datasets/`, `methods/`, and `sidecars/` children, for signature changes while the page is open. The initial cached dataset table load shares its `/datasets/cached` response with the watcher baseline instead of starting a second folder scan for the same path. When disk changes add or remove unique cached instance names, it applies the new cached-dataset snapshot and refreshes the right-panel dataset table automatically. Content-only edits that keep the same instance names update the watch baseline without disrupting the table.
...
- Active-path disk-change detection compares the cached folder signature for the selected path at an interval, then compares the unique cached instance-name set before refreshing the cached dataset lookup and table state.
