...
- `ui/project_instance/project_instance.html`: external scripts `/ui/project_instance/project_instance.js?v=20260616a`; inline imports _none_.
...
- Double-clicking a vector dataset row whose `Method Type` is `DFM`, or choosing `View` from its row context menu, opens the DFM method tab by default. The same row context menu shows `Show as vector` immediately after `View` for DFM vector rows; choosing it opens the saved DFM result in a normal Dataset Viewer window without showing DFM method details.
