...
- `ui/project_instance/project_instance.html`: external scripts `/ui/project_instance/project_instance.js?v=20260617a`; inline imports _none_.
...
- The dataset table `Method Type` column reads method metadata from the selected reserving-class folder's cached `index.json` `files` entries, matching rows by instance `dataset_name`; datasets missing from the cache display `None`.
...
- Method type lookup is derived from the same selected-path `index.json` payload as cached dataset names and metadata, so Project Instance does not issue a second method-index request during path selection.
