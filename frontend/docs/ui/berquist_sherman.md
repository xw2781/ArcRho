...
- Number formats follow the canonical `dataset_number_format.js` helpers. Each input view renders with its own dataset instance's `number_format`/`decimal_places`, exactly as the Dataset Viewer shows that dataset; every calculated triangle uses the Details tab's `Number Format` and `Decimal Places` pair, which is also the output dataset's own format and is written to the output sidecar on save. B&S applies the Dataset Viewer mask together with the annual staircase when source triangles load, so real zero observations remain visible while padded zeroes in the unavailable lower-right area stay blank in source and calculated views. Two rate grids are exempt: `Proportion Settled` keeps its percentage presentation and the CRA `Average Inflation` grid its four decimals, because at the Details format's zero or two decimals every estimator in a column would print the same figure.
