...
- The Details tab stacks the object fields above the source fields in two full-width panels without separate panel-header bars. Its contents take their label and field look from the shared Details primitive `details_form_layout.css`, which the page stylesheet must not restate; see the Frontend Behavior Contract. The `Project` row is host-fixed and hidden outside a Workflow step, so the tab opens on Reserving Class.
