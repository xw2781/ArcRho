...
- `ui/method_pages/bornhuetter_ferguson/bornhuetter_ferguson.html`: external scripts `/ui/method_pages/bornhuetter_ferguson/bornhuetter_ferguson_main.js?v=20260817a`, `/ui/shared/services/color_theme.js?v=20260811a`; inline imports _none_.
...
- The Details tab is one evenly spaced, ungrouped field list in the shared order: Name, Output Type, Project, Segment, Origin Length, Latest, Development Pattern, and Prior Vector. `Segment` is this page's label for the reserving-class path. Its contents take their label and field look from the shared Details primitive `details_form_layout.css`, which the page stylesheet must not restate; see the Frontend Behavior Contract. The `Project` row is host-fixed and hidden outside a Workflow step, so the tab opens on Name.
