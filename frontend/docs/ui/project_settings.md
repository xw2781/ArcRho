...
- Source Data shows the source identity without a decorative state dot, with a borderless circled-exclamation details icon immediately to its right. For a SQL Server project the identity is the table name over `server · database`. Loading and error feedback remains in the inline message area. Its responsive floating `Source Table` panel is up to 420px wide and shows the source folder with an inline copy action. Clicking the identity opens the Import Settings window rather than an inline path editor; the hidden path input remains the coordinator's record of the current CSV selection, which the folder action reads and the window commits through. Open folder uses the desktop host bridge.
...
- A CSV selection is still written to the project-map `Table Path` column that field mapping owns; only the SQL Server profile and the chosen method go to the project's import record.
