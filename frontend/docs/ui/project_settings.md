...
- `ui/project_settings/project_settings.html`: external scripts `/ui/project_settings/project_settings.js?v=20260807idx1`, `/ui/shared/services/color_theme.js?v=20260724a`; inline imports _none_.
...
- Source Data shows the source identity without a decorative state dot, with a borderless circled-exclamation details icon immediately to its right. For a SQL Server project the identity is the table name over `server · database`. Loading and error feedback remains in the inline message area. Its responsive floating `Source Table` panel is up to 420px wide and shows the source folder with an inline copy action. Clicking the identity opens the Import Settings window rather than an inline path editor; the hidden path input remains the page's record of the current CSV selection, filled from `/source_table` responses and read by the folder action. Open folder uses the desktop host bridge.
- The Source Data header keeps three icon actions: Reload, Import settings, and Open folder. The former Copy-path action and the per-method settings/import icons were removed; one window now covers both import methods.
...
- Source Data header controls (details icon, Reload, Import settings, Open folder) use the shared ArcRho tooltip surface, labelled from each control's own `aria-label`. Controls inside the floating panels and the Import Settings window are deliberately excluded, because the shared tooltip's `z-index: 5600` sits below them.
