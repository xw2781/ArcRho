...
- `ui/project_settings/project_settings.html`: external scripts `/ui/project_settings/project_settings.js?v=20260717dprphase2`; inline imports _none_.
...
- The Reserving Class Types editor has one Formula frame and component tray. Normal Formula `+`/`-` terms define signed base aggregation coefficients for every source measure. The retired `EEX Formula` column is rejected on legacy JSON/XLSX load until the explicit data-processing-rules migration is run.
- The Data processing rules ribbon page provides add/edit/duplicate/delete, enable/disable, server validation, and read-only JSON review. Rules use the standard resizable Project Settings table with Enabled, Name, Dataset type, Applies when, and Effect columns. Each row has a compact enabled switch, and right-clicking a row provides Edit, Duplicate, Enable/Disable, and Delete.
