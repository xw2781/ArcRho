...
The desktop shell hosts ArcBot through the shared `ui/ai-assistant/arcrho.js` adapter.
...
- `arcrho:dataset-close-confirm-ack`
...
- Clear Cache & Reload is available from the Settings menu and `Alt+Q`. It asks open Project Instance tabs for a current state snapshot, stores a one-shot shell restore payload in the Electron host, clears Electron cache/storage, then reloads the shell with a fresh timestamped UI URL and restores the previously active page/tab layout before normal localStorage persistence resumes. File > Restart uses the same Electron cache/storage clear and restore snapshot before posting the app restart requests, so users do not need to run Clear Cache & Reload separately before restarting. If the desktop host cannot perform the cache reload, the shell reports that status and falls back to a browser reload. Project Settings iframes include the shell UI version query parameter so reloads fetch the current Project Settings HTML/module graph consistently.
...
- Packaged ArcRho and Arcode installers include the bundled `node-portable` runtime as an Electron resource so ArcBot can use its bundled `npm` and `codex` commands without requiring a user-installed Node.js. After a Codex CLI install completes, the Electron host stores the npm-discovered `codex` command path for later status checks; if the install succeeds but the current app process still cannot resolve the command, ArcBot shows a restart/refresh recovery message instead of reopening the install prompt.
...
- The shell polls the local `/ui_automation/commands/poll` app-server endpoint for Python UI automation commands. It executes typed commands such as `ui.messageBox` in the shell with a draggable floating message window, drives `ui.progressOpen`/`ui.progressUpdate`/`ui.progressClose` progress dialogs with a compact label, count/percent, progress bar, close icon, and resize handle for long-running macros, routes `projectInstance.openDataset`, `projectInstance.refreshDatasets`, and Project Instance window-object commands to the active Project Instance iframe using explicit automation messages, relays `taskDesigner.*` commands to the Task Designer tab iframe, and posts structured results back through the app-server command bridge.
- The shell handles Dataset dirty-close confirmation requests with the same high-layer floating message surface used by automation dialogs, then returns the discard/cancel result to the requesting Dataset iframe or Project Instance relay.
