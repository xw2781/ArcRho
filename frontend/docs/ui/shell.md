...
- The menubar includes `Macro` after Settings. Clicking it opens a shell-owned floating macro panel owned by `ui/macro/macro_window.js`, with the same in-app titlebar drag behavior used by the ArcBot panel plus resize handles on every edge and corner; it lists macro names for `.py` files from the user scripting directory plus non-runnable demo examples, shows the selected macro description in a detail pane with a draggable splitter, reports run/edit/delete results through the app status bar, and can run real selected macros against the active DFM tab/window, open them in Arcode for code viewing/editing, or delete user-created macros from the scripting folder after confirmation. Right-clicking the splitter opens a Macro actions menu with Run, Edit, and Delete. Macros can optionally return a Notes diff preview; the shell shows a DFM RPC-style review dialog and applies the macro payload only after the user accepts the corrected Notes.
