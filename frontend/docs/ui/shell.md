...
- The menubar includes `Macro` after Settings. Clicking it opens a shell-owned floating macro panel owned by `ui/macro/macro_window.js`, with the same in-app titlebar drag behavior used by the ArcBot panel plus resize handles on every edge and corner; it lists macro names for `.py` files from the user scripting directory plus non-runnable demo examples, shows the selected macro description in a detail pane with a draggable splitter, reports run/edit/delete results through the app status bar, and can run real selected macros against the active DFM tab/window, open them in a new Scripting Console tab for code viewing/editing, or delete user-created macros from the scripting folder after confirmation. Right-clicking the splitter opens a Macro actions menu with Run, Edit, and Delete. The seeded Apply Growth Adjustments macro marks each pre-adjustment selected average cell with a DFM summary-table cell note before writing the adjusted user ratio, stores adjusted User Entry values at six-decimal precision, formats note multipliers compactly, and does not require existing actuary notes before applying workbook adjustments. Macros can optionally return a Notes diff preview; the shell shows a DFM RPC-style review dialog and applies the macro payload only after the user accepts the corrected Notes.
...
- In the desktop app, dragging files from File Explorer over the shell or an open feature iframe shows a shell-owned drop overlay. Dropping `.ipynb`, `.arcnb`, or `.py` files opens each scripting file in a fresh Scripting Console tab, then forwards the disk path to that tab with `arcrho:scripting-open-path`.
