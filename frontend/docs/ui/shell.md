...
- `ui/index.html`: external scripts `/ui/shell/ui_shell.js?v=20260531a`; inline imports _none_.
...
- After the desktop shell is visible, the Electron host checks `E:\ArcRho Server\installer` for a newer ArcRho installer without blocking startup. The check times out quickly, requires a SHA-256 checksum before launching an installer, asks the user whether to update, and reuses the normal shutdown confirmation before starting the installer and quitting ArcRho.
