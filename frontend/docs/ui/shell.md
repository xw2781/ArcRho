...
- `ui/index.html`: external scripts `/ui/shell/ui_shell.js?v=20260620h`; inline imports _none_.
...
- After a Codex CLI install completes, the Electron host stores the npm-discovered `codex` command path for later status checks; if the install succeeds but the current app process still cannot resolve the command, ArcBot shows a restart/refresh recovery message instead of reopening the install prompt.
...
- The menubar includes `Macro` after Settings. Clicking it opens a shell-owned floating macro panel owned by `ui/macro/macro_window.js`, with the same in-app titlebar drag behavior used by the ArcBot panel plus resize handles on every edge and corner; it lists macro names for `.py` files from the user macros directory `Documents\ArcRho\macros` plus non-runnable demo examples, shows the selected macro description in a detail pane with a draggable splitter, reports run/edit/delete results through the app status bar, and can run real selected macros against the active DFM tab/window, open them in Arcode for code viewing/editing, or delete user-created macros from the macros folder after confirmation. Right-clicking the splitter opens a Macro actions menu with Run, Edit, and Delete. Macros can optionally return a Notes diff preview; the shell shows a DFM RPC-style review dialog and applies the macro payload only after the user accepts the corrected Notes. UI-only macros can also run without an active DFM and can use `arcrho_api.ui` helpers to show shell message boxes or dispatch typed window commands.
