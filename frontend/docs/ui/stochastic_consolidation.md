...
- A header bar above the tabs holds the method name, a run-state chip (`Not run yet`, `Consolidating` while a run is in flight, `Up to date`, `Inputs changed — run again`, or `A segment changed`), the run's simulation count and seed (plus how long the last run in this window took, and "not saved yet" for a run from Consolidate), and Consolidate, Save and Cancel.
...
- **Results** reads only the stored summary. A consolidation combines scaled reserves only, so there is a `Reserves / Ultimates` switch but no `Scaled / Unscaled` one; the percentile chooser, Full Ladder, Copy, the summary table (without the DFM columns a bootstrap has), the distribution chart and the fan chart behave exactly as on the Bootstrap page. Below them the Segment Breakdown lists each segment's standalone mean, standard deviation and CV, its factor and its share of the total mean, with a total row and a caption giving the total's standard deviation against the sum of the standalone ones, which is the diversification the correlations give. A notice with Consolidate appears while the run is stale, and until a run exists the tab shows an empty state with Consolidate.
