...
- On Dataset page open, loads the base generated dataset sidecar `<DatasetName>.json` for the selected project/reserving-class/dataset and applies its saved `origin_length`, `development_length`, cumulative, transposed, and calendar Data-tab settings before loading the triangle. If the saved lengths differ, the page disables the visual link toggle so both saved values are preserved.
