...
test("completion tracks nested arguments without counting quoted commas or array separators", () => {
  ...
  assert.equal(formulaCallContext('=ArcoTri("A,B", "Paid", TRUE, ').argument, 3);
  assert.equal(formulaCompletionQuery('=SUM([Pa').kind, "dataset");
  assert.equal(formulaCompletionQuery('=ArcoVec("", "Pa').kind, "datasetArgument");
  assert.equal(formulaCompletionQuery('=ArcoVec("[foo').kind, "contextArgument");
...
test("completion replaces the token at the caret and preserves the remaining expression", () => {
  ...
  assert.equal(completionEdit(text, query, '"Paid"').value, '=ArcoVec("", "Paid") * 2');
  text = '=ArcoVec("Other RC", "Pa old",,"Other Project")';
  query = formulaCompletionQuery(text, text.indexOf('Pa') + 2);
  assert.equal(completionEdit(text, query, '"Paid"').value, '=ArcoVec("Other RC", "Paid",,"Other Project")');
...
test("add-in calls accept omitted context and optional slots and escaped text", () => {
  const raw = '=INDEX(ArcoTri(,"Paid ""Net""",,,,"Other Project"),2,1)';
  ...
  assert.equal(findArcRhoFormulaReferences('="ArcoVec(ignored)"').length, 0);
  assert.equal(findArcRhoFormulaReferences('=[ArcoVec(fake)][1]').length, 0);
});

test("preview and DSV evaluation use one batched resolver and produce the same array", async () => {
  let calls = 0;
  const result = await evaluateFormulaValues('=TRANSPOSE(TAKE(ArcoVec(,"Paid"),-2))', {
    resolveReferences: async refs => {
      calls++; assert.deepEqual(refs, ['=ArcoVec(, "Paid")']);
