...
class ArcRhoFormulaTests(unittest.TestCase):
    ...
    def test_empty_context_defaults_and_remote_scope_keep_shape_options(self):
        self.values('ArcRhoTri(,"Paid",FALSE,,TRUE,,6,3)')
        ...
        self.values('ArcRhoVec("Other/RC","Premium",FALSE,"Other",6)')
        pairs = dict(self.reader.call_args.args[0])
    ...
    def test_related_functions_and_transposition_share_one_read_per_shape(self):
        cache = {}
        self.assertEqual(self.values('ArcRhoTriCell(,"Paid",2,1)', cache), [[40]])
        self.assertEqual(self.values('ArcRhoTriOrigin(,"Paid",1,,TRUE)', cache), [[10], [20], [30]])
        self.assertEqual(self.values('ArcRhoTriDiag(,"Paid")', cache), [[30], [50], [60]])
        self.assertEqual(self.values('ArcRhoTriDiag(,"Paid",-1)', cache), [[20], [40], [0]])
        self.assertEqual(self.reader.call_count, 1)
        self.reader.return_value = {"ok": True, "csv_text": "10\n20\n30\n"}
        self.assertEqual(self.values('ArcRhoVecCell(,"Premium",2)'), [[20]])
        self.assertEqual(self.values('ArcRhoVec(,"Premium",TRUE)'), [[10, 20, 30]])

    def test_invalid_coordinates_and_read_failures_are_visible(self):
        for formula in ['ArcRhoVecCell(,"V",0)', 'ArcRhoTri(,"T",,,,,0)', 'ArcRhoTriCell(,"T",99,1)']:
        ...
        with self.assertRaisesRegex(HTTPException, "Dataset not found"): self.values('ArcRhoVec(,"Missing")')
    ...
    def test_dfm_recalculation_resolves_saved_index_formulas_with_remote_context(self):
        ...
        raw = '=INDEX(TRANSPOSE(ArcRhoVec("Other/RC","Factors",,"Other")),1,2)'
    ...
    def test_local_edges_use_full_context_and_remote_names_do_not_alias_local_datasets(self):
        formulas = [{'formula': '=ArcRhoVec("Current/RC","Local",,"Current")'}, {'formula': '=ArcRhoVec("Other/RC","Remote",,"Other")'}]
    ...
    def test_call_text_inside_dataset_names_is_not_a_function(self):
        self.assertEqual(arcrho_formula_references('=[ArcRhoVec(fake)][1]'), [])
    ...
    def test_formula_route_requires_gateway_and_preserves_transport_errors(self):
        ...
        request = router.DatasetInternalLinksResolveRequest(project_name="Current", reserving_class="RC", references=['=ArcRhoVec(,"Paid")'])
