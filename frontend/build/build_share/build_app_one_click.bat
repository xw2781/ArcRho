...
REM Requests a fresh source ZIP from the source-PC listener, then runs the
REM established local-workspace build wrapper and opens its published installer.
...
echo.
echo Starting the standard local-workspace ArcRho build...
call "%LOCAL_BUILD_WRAPPER%" %*
set "BUILD_EXIT_CODE=%ERRORLEVEL%"
if not "%BUILD_EXIT_CODE%"=="0" (
    echo ERROR: Automated ArcRho build failed with exit code %BUILD_EXIT_CODE%.
