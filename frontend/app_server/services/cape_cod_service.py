...
def _read_source_snapshot_from_sidecar(
    ...
    *,
    ...
    if period and period != origin_length:
        raise HTTPException(
            422,
            f"Cape Cod precedent '{requested_name}' uses {period}-month origins; expected {origin_length}.",
        )
...
def _refresh_one(
    ...
    *,
    ...
    if blocked:
        raise RuntimeError("Required Cape Cod precedent needs review: " + ", ".join(blocked))
