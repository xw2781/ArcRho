...
def _read_source_snapshot_from_sidecar(
    ...
    *,
    ...
    if role == "dfm":
        snapshot["percentage_developed"] = _dfm_percentage_developed(
            project_name, reserving_class, sidecar, requested_name, len(method_origin_labels)
        )
    return snapshot


def _dfm_percentage_developed(
    project_name: str,
    reserving_class: str,
    sidecar: Mapping[str, Any],
    requested_name: str,
    ...
    """Read the development pattern behind a DFM output, as a percentage per origin.

    The percentage a BF applies is a property of the DFM's selected development
    factors, so it is read from the DFM method rather than divided out of the
    published ultimates. Dividing cannot describe an origin whose latest figure
    is zero, and the DFM knows that origin's development age regardless.
    ...
    if len(values) != row_count:
        raise HTTPException(
            422,
            f"BF Development Pattern '{method_name}' has {len(values)} origins; expected {row_count}.",
        )
    return values
...
def save_propagation_roots(
    ...
    **_ignored: Any,
    ...
    """Return the roots ``save_bornhuetter_ferguson_method`` propagates from.
    ...
    refuses to change a BF method's output identity (409), so the incoming
...
def _refresh_one(
    ...
    *,
    ...
    if blocked:
        ...
        raise RuntimeError("Required BF precedent could not be refreshed: " + ", ".join(blocked))
...
def refresh_output(
    ...
    """Republish one Bornhuetter-Ferguson output and report instead of raising.
    ...
    sidecar_cache = dependent_walk_service.walk_cache(caches, "bornhuetter_ferguson_sidecars")
    snapshot_cache = dependent_walk_service.walk_cache(
        caches, "bornhuetter_ferguson_source_snapshots"
    )
...
def refresh_dependents(
    ...
    *,
    ...
    """Refresh what these datasets reach and report the Bornhuetter-Ferguson part of it.
